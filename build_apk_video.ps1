...
$PROJ_DIR = (Get-Item "android_video_project").FullName
$OUT_DIR = "$PROJ_DIR\out"
$ASSETS_DIR = "$PROJ_DIR\app\src\main\assets"
$UNALIGNED_APK = "$OUT_DIR\apk\app-unaligned.apk"
$FINAL_APK = (Join-Path (Get-Location) "CodigoCarta_Video_Revelacion.apk")

Write-Host "Iniciando compilacion de Codigo Carta Video Revelacion APK..." -ForegroundColor Cyan
...
Write-Host "[2/5] Enlazando APK con 52 videos en assets (aapt2 link -A assets)..."
...
& "$JAVA_HOME\bin\keytool.exe" -genkeypair -keystore $keyStore -storepass android -alias androiddebugkey -keypass android -dname "CN=CodigoCartaVideo,O=Mentalismo,C=ES" -keyalg RSA -keysize 2048 -validity 10000
